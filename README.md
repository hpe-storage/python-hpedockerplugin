...
+ The Ansible installer only supports Ubuntu/RHEL/CentOS. 
...
+ Upgrade of existing Docker engine to higher version might break compatibility of HPE Docker Volume Plugin.
...
5. Firewalld service should be off during Kubernetes setup.
...
```
7. Set SSH connection with 3PAR
...
```
**Note:** It is recommended that the properties file is [encrypted using Ansible Vault](https://github.com/hpe-storage/python-hpedockerplugin/blob/master/ansible_3par_docker_plugin/encrypt_properties.md)
